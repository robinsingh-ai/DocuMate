...
LOCAL_PDF_DIRECTORY = '/Users/robinsingh/Downloads/v1-201-level3-multiagent-p9-reviewed-master 3/backend/local_pdfs'
...
@retry(stop=stop_after_attempt(3), wait=wait_fixed(5), retry=retry_if_exception_type(RateLimitError))
def invoke_qa_chain(qa_chain, question):
...
def chat_pdf():
    ...
    try:
        ...
        answer = invoke_qa_chain(qa_chain, question)
        print("QA chain response received")
        return jsonify({
            "answer": answer,
            "sources": []  # Note: This setup doesn't return source documents
        }), 200
    except Exception as e:
...
def setup_qa_chain(session_id):
    ...
    try:
        ...
        # Use a smaller, faster model for initial retrieval
        retrieval_llm = ChatOpenAI(temperature=0, model='gpt-3.5-turbo')

        # Set up the base retriever with a smaller k
        ...
        query_prompt = PromptTemplate(
            ...
            template="Rephrase the following question for better search results:\n{question}\n\nRephrased question:"
        )
        ...
        # Define a compact prompt for the QA chain
        qa_prompt = PromptTemplate(
            input_variables=["context", "question"],
            template="Answer the question based on the context. If the context doesn't contain relevant information, say 'I don't have enough information to answer this question.'\nContext: {context}\nQuestion: {question}\nAnswer:"
        )

        # Text splitter for context
        text_splitter = CharacterTextSplitter(chunk_size=1000, chunk_overlap=0)

        # Set up the QA chain
        qa_chain = (
            {"context": retriever | (lambda docs: text_splitter.split_text(docs[0].page_content) if docs else []), "question": RunnablePassthrough()} 
            | qa_prompt 
            | llm 
            | StrOutputParser()
        )
